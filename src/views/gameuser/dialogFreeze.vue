...
<div>
  <el-form  :model="form" :inline="true"  :rules="rules" ref="form" label-width="100px" class="demo-ruleForm">
        ...
        <el-form-item label="操作类型"  prop="nullity">
        ...
        <el-form-item label="操作原因"  prop="reason">
          <el-input v-model.trim="form.reason" size="small"  placeholder="输入操作类型的原因" autocomplete="off" clearable></el-input>
        </el-form-item>
      </el-form>
  ...
  methods: {
    ...
    doSubmit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.loading =true;
          this.$api.updateAccountsInfo(this.form).then(res =>{
            this.loading = false;
            if (res.success) {
              this.resetForm();
              this.$message({
