    ...
    <el-row :gutter="32" style="margin-bottom:10px;">
      <el-form ref="form" :model="form">
        ...
        <el-col :xs="12" :sm="6" :lg="4">
          <el-date-picker v-model="form.beginTime" size="small" align="right" type="date" placeholder="开始日期" value-format="yyyy-MM-dd" format="yyyy-MM-dd" clearable></el-date-picker>
        </el-col>
        <el-col :xs="12" :sm="6" :lg="4">
          <el-date-picker v-model="form.endTime" size="small" align="right" type="date" placeholder="结束日期" value-format="yyyy-MM-dd" format="yyyy-MM-dd" clearable></el-date-picker>
        </el-col>
        <el-col :xs="12" :sm="6" :lg="3">
          <el-select v-model="form.propType" size="small" placeholder="方式">
            <el-option v-for="list in selectList.propTypelist" :key="list.key" :value="list.key" :label="list.value"></el-option>
          </el-select>
        </el-col>
        <el-col :xs="24" :sm="12" :lg="6" v-has="'itemRecord_search'">
    ...
    <el-row>
      <el-col :span="24">
        <el-table :data="tableData" style="width: 100%" :cell-style="cellStyle" :header-cell-style="rowClass" border v-loading="tableloading">
          ...
          <el-table-column prop="itemName" label="道具" show-overflow-tooltip>
          ...
          <el-table-column prop="room" label="房间" show-overflow-tooltip>
          ...
          <el-table-column prop="earn" label="获得幸运豆" show-overflow-tooltip>
            <template slot-scope="scope">{{ scope.row.earn | numTenthousand }}</template>
          </el-table-column>
          <el-table-column prop="operationName" label="来源" show-overflow-tooltip></el-table-column>
          <el-table-column prop="operatetime" label="操作时间" width="160px"></el-table-column>
        </el-table>
...
<script>
    export default {
        ...
        methods:{
            getParams() {
              // 取到路由带过来的参数  begin,end,itemId,type
              this.form.propid = this.$route.query.itemId;
              this.form.propType = this.$route.query.type;
              ...
              if(this.form.propid == "" || this.form.propid ==null || typeof(this.form.propid) == undefined){
            ...
            onSubmit(pageNum){
              //userid, itemid, room, operation(看后边的列), beginTime, endDate, currentPage, limit
              let param = {
                userid:this.form.userid,
                itemid:this.form.propid,
                room:this.form.roomid,
                operation:this.form.propType,
              ...
              this.$api.itemRecordsearch(param).then(res =>{
