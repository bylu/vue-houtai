<template>
    <div class="channelpaydatacontent">
      <h3 class="showTit" style="margin-left:0px;">游戏付费分析表格</h3>
       <el-row :gutter="32">
        <div style="width:100%">
            <el-button size="mini"  type="primary"  @click="searchData(7)" style="margin-left:1%">7天</el-button>
            <el-button size="mini"  type="danger"   @click="searchData(30)">30天</el-button>
            <el-button size="mini"  type="success"  @click="searchData(60)">60天</el-button>
        </div>
       </el-row>
        <template>
          <el-table
...
export default {
  ...
  methods: {
    ...
    searchData(data) {
      ...
      this.$api.spreaderAchievementSearchDayBoss(params).then(res =>{
         this.loading = false;
        let rs = new Array();
        let sumPayAmount = 0;
        ...
        if (res.success) {
         if(res.data.length > 0){
            ...
            let arpu = (payAmount / payUsers).toFixed(2);
            //let totalArpu = (totalPayAmount / totalUser).toFixed(2);
            let totalArpu = isFinite(totalPayAmount / totalUser) ?  (totalPayAmount / totalUser).toFixed(2):'0.00';
            let dau = val.dau;
            // let dauPru = (payUsers / dau).toFixed(2);
            // let dauApru = (payAmount / dau).toFixed(2);
