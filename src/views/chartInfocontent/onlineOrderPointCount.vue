<template>
  <div class="app-container">
    <h3 class="showTit">计费点统计</h3>
    <el-row :gutter="32">
      <el-form ref="form" :model="form">
        <el-col :xs="24" :sm="6" :lg="3">
          <el-select v-model="form.spreaderType" placeholder="请选择" size="small" >
           <el-option v-for="list in selectList.spreaderTypelist" :key="list.key" :value="list.key" :label="list.value"></el-option>
          </el-select>
        </el-col>
        ...
        <el-col :xs="24" :sm="24" :lg="8" v-has="'onlineOrderPointCount_search'">
    ...
    <el-row>
      <el-col :span="24">
        <el-table :data="tableData" style="width: 100%" :cell-style="cellStyle" :header-cell-style="rowClass" border v-loading="tableloading">
          ...
          <el-table-column prop="propType" label="类型"></el-table-column>
          <el-table-column  label="充值次数">
              <template  slot-scope="scope">
                  {{scope.row.payCount | null2Zero}}
              </template>
          </el-table-column>
          <el-table-column  label="充值累计金额">
              <template  slot-scope="scope">
                  {{scope.row.sumPayAmount | null2Zero}}
              </template>
          </el-table-column>
          <el-table-column label="充值次数占比">
              <template  slot-scope="scope">
                  {{scope.row.payCountTimes | formatDouble}}
              </template>
          </el-table-column>
          <el-table-column prop="sumPayAmountTimes" label="充值金额占比">
              <template  slot-scope="scope">
                  {{scope.row.sumPayAmountTimes | formatDouble}}
              </template>
          </el-table-column>
        </el-table>
...
<script>
    export default {
        ...
        methods:{
            ...
            onSubmit(pageNum){
              let param = {
                spearderType:this.form.spreaderType,
              ...
              this.$api.onlineOrderPointCountSearch(param).then(res =>{
