<template>
  <div>
    <el-form :model="form" :inline="true" ref="form" :rules="rules"  label-width="60px" class="demo-ruleForm">
        ...
        <el-form-item label="answerId:" prop="answerId"  style="display:none">
          <el-input v-model.trim="form.answerId" size="small" autocomplete="off" ></el-input>
        </el-form-item>
       
        <el-form-item label="问题" prop="answerContent">
          <el-input v-model="form.answerContent" size="small"  type="textarea" style="width:200%" :autosize="{ minRows: 4, maxRows: 4}" autocomplete="off" ></el-input>
        </el-form-item>
...
export default {
  ...
  methods: {
    init(){
        ...
        this.form.answerId=this.getdata.answerId;
        this.form.answerContent=this.getdata.answerContent;
    },
    //取消按钮操作
    cancel() {
      this.resetForm();
    ...
    doSubmit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          this.$api.surveyupdateChildAwswer(this.form).then(res =>{
            this.loading = false;
            if (res.success) {
              this.resetForm();
              this.$message({
