...
<div>
   <el-form
        ...
        <el-form-item label="上传文件:"  prop="fileName">
          <!-- <el-input v-model="fileName" size="small"  :readonly='true' placeholder="文件名"></el-input> -->
           <!-- <el-upload
            class="upload-demo"
            ref="upload"
            action="doUpload"
        ...
        <el-form-item> -->
          ...
          <div class="showFileName">{{showFilename}}</div>
  ...
  methods: {
    ...
    uploadFile(event) {
      if(event==null){
        ...
        let file = event.target.files[0]; //获取input的file值
        ...
        this.showFilename="文件上传中。。。";
        let param = new FormData(); // 创建form对象
        param.append("file", file);
        this.$api.itemMapimport(param).then(res =>{
          if (res.success) {
    ...
    btnSubmit(){
      ...
      this.$api.itemMapimportbegin().then(res =>{
